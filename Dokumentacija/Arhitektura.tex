...
\begin{longtblr}[
	...
	entry=none
		...
		colspec={|X[6,l]|X[6,l]|X[20,l]|},
		rowhead = 1,
		hlines, vlines, % Use hlines and vlines for horizontal and vertical lines
	}
	\SetCell[c=3]{c}\textbf{Social Media} \\ 
	\SetRow{bg=LightGreen} userId & INT & jedinstveni identifikator korisnika \\
...
\begin{longtblr}[
	...
	\SetCell[c=3]{c}\textbf{Privacy Settings} \\ 
	\SetRow{bg=LightGreen} userId & INT & jedinstveni identifikator korisnika \\
...
Dijagram komponenti prikazuje arhitekturu sustava i način na koji se komponente međusobno povezuju. Frontend dio aplikacije izgrađen je pomoću React biblioteke, gdje komponente upravljaju prikazom korisničkog sučelja te se koriste za dinamičko prikazivanje sadržaja koristeći HTML, CSS i JavaScript datoteke. React-view komunicira s Codeblaze aplikacijom preko dostupnih sučelja te na korisnikove zahtjeve osvježava prikaz. Modulu REST API, koji poslužuje podatke koji pripadaju backendu, pristupa se preko sučelja za dohvat JSON podataka. Za dohvaćanje podataka iz baze pomoću SQL upita, zaduženi su repozitoriji. Servisni sloj prima podatke iz repozitorija i može ih dodatno obraditi. Zatim se podaci prosljeđuju kontroleru koji šalje odgovor frontendu u JSON formatu.
